/**
 * The icon resolution engine: a port of the VSCode better-sidebar-icons engine to
 * plain functions (the same logic dsh-better-sidebar's core explorer icon
 * theme uses, kept dependency-free here so the client overlay can resolve
 * entry names without React).
 *
 ...
 * bundled default icons. Bytes are served by the plugin's /dsh-better-sidebar-icons/icons
 * route; this module only RESOLVES an entry name to an icon file:
 *
 ...
 *   first, each exact then permissive. The generated maps already fold the
...
/** The basename of a path (both separators), falling back to the input. */
...
/** The plugin route serving the icon set (same origin, relative URL). Keep
 *  in sync with src/icons-route.ts ICON_ROUTE_PREFIX. */
export const ROUTE_BASE = '/dsh-better-sidebar-icons/icons'

/** The absolute URL of one icon file on the plugin route. */
export function iconUrl(file: string): string {
  return `${ROUTE_BASE}/${encodeURIComponent(file)}`
}
